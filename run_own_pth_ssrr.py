...
from util.MF_dataset_SSRR import MF_dataset
from util.util import compute_results, visualize
from model.U_Net_CBAM_V2 import UNet

""
parser = argparse.ArgumentParser(description='Test with pytorch')
""
parser.add_argument('--model_name', '-m', type=str, default='UNet_CBAM')
parser.add_argument('--weight_name', '-w', type=str, default='checkpoint_UNet_CBAM_IV')
parser.add_argument('--file_name', '-f', type=str, default='./checkpoint_UNet_CBAM.pt')
parser.add_argument('--dataset_split', '-d', type=str, default='selection')  # test, test_day, test_night
...
parser.add_argument('--data_dir', '-dr', type=str, default='/workspace/RGB_T_SAR/rgb_est/')
parser.add_argument('--model_dir', '-wd', type=str, default='/workspace/RGB_T_SAR/SAR_Nets/')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for it, (images, names) in enumerate(test_loader):
            ...
            # save demo images
            #if not os.path.exists('./visual_label'):
            #    os.makedirs('./visual_label')
            #visualize(image_name=names, predictions=labels, weight_name='Pred_' + args.weight_name)
            if not os.path.exists('./SSRR_video/' + 'result/Pred/' + args.weight_name +'/'):
                os.makedirs('./SSRR_video/' + 'result/Pred/' + args.weight_name+'/')
    ...
    conf_total_matfile = os.path.join('./UNet_coding/result/Pred_' + args.weight_name, 'conf_' + args.weight_name + '.mat')
    print('\n###########################################################################')
    ...
    print(
        "* recall per class: \n    unlabeled: %.6f, first_responder: %.6f, civilian: %.6f, vegetation: %.6f, road: %.6f, dirt_road: %.6f, building: %.6f, sky: %.6f, civilian_car: %.6f, responder_vehicle: %.6f, debris: %.6f, command_post: %.6f" \
        % (recall_per_class[0], recall_per_class[1], recall_per_class[2], recall_per_class[3], recall_per_class[4],
           ...
           recall_per_class[10], recall_per_class[11]))
    print(
        "* iou per class: \n    unlabeled: %.6f, first_responder: %.6f, civilian: %.6f, vegetation: %.6f, road: %.6f, dirt_road: %.6f, building: %.6f, sky: %.6f, civilian_car: %.6f, responder_vehicle: %.6f, debris: %.6f, command_post: %.6f" \
        % (iou_per_class[0], iou_per_class[1], iou_per_class[2], iou_per_class[3], iou_per_class[4], iou_per_class[5],
           iou_per_class[6], iou_per_class[7], iou_per_class[8], iou_per_class[9], iou_per_class[10], iou_per_class[11]))
